...
EXPECT_THINKING_BUDGET_REJECTED = os.environ.get("EXPECT_THINKING_BUDGET_REJECTED", "1") == "1"
...
def t_thinking_budget_rejected():
    try:
        r = chat("Hej", max_tokens=8, thinking_token_budget=10)
        return (not EXPECT_THINKING_BUDGET_REJECTED), (
            "accepted by V1 batch runner"
            if not EXPECT_THINKING_BUDGET_REJECTED
            else "accepted (unexpected for V2 runner)"
        )
    except urllib.error.HTTPError as e:
        ok = e.code == 400 if EXPECT_THINKING_BUDGET_REJECTED else False
        return ok, f"HTTP {e.code} ({'expected' if ok else 'unexpected'} for selected runner)"
