...
# venv + vLLM version, core and integration patches applied, the model
# requantized (lm_head, embed_tokens, MTP module, draft head), keys/files
# present, and — if a
...
EOF
command -v nvidia-smi >/dev/null && { PL=$(nvidia-smi --query-gpu=power.limit --format=csv,noheader,nounits | head -1); ok "power limit ${PL} W (checked against the selected profile by the host launcher)"; }
fi
...
echo "== Codex/Responses integration patches (integrations/codex/)"
...
$PY integrations/codex/test_responses_tools.py >/dev/null 2>&1 \
  && ok "Responses tool-tail and single-call streaming regressions" \
  || fail "Responses tool-tail or single-call streaming regression"
...
if [ $NOSRV = 0 ]; then
  ...
  else warn "no server on :$PORT (use scripts/run-single-user.sh or scripts/run-batch.sh, or pass --no-server)"; fi
fi
echo
